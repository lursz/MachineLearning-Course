...
class SearchProcess():
    ...
    def expand_frontier(self, upper_bound: float, opposite: SearchProcess) -> Optional[float]:
        ...
        next_node = self._select_candidate(upper_bound, opposite)
        if next_node is None:
            return None
        # 2. Mark the node as scanned (add it to the self.scanned_states)
        self.scanned_states.add(next_node.state)
        # 3. Update the lower bound if the estimated value for the state is lower than the current one
        self.cost_lower_bound = min(self.cost_lower_bound, self._estimated_cost(next_node))
        ...
        if opposite.tree.root != node.state:
            for child in self.tree.expand(self.problem, next_node):
                if child.state not in self.rejected_states or child.state not in self.scanned_states:
                    if self.labeled_states[child.state] > child.cost:
                        self.labeled_states[child.state] = child
                        self.frontier.push(child)
                        upper_bound = min(upper_bound, child.cost + opposite.heuristic(child.state))
    ...
    def _select_candidate(self, 
        ...
        while self.frontier:
            ...
            if self._estimated_cost(node) <= upper_bound:
                if self._estimated_cost(node) + opposite.labeled_state_cost(node.state) <= upper_bound or self.labeled_state_cost(node.state) + opposite.cost_lower_bound - opposite.heuristic(node.state) <= upper_bound:
                    return node
